...
def formatter(i, list, lines, x, y, x_to_family, family_to_special):
    if list:
        for i in list:
            s = "  " + "{:<12}".format(x[i])
            s += " =  " + "{:<20}".format(y[i]) 
            special_i = family_to_special[x_to_family[i]]
            s += " in family " + "{:<12}".format(x[special_i - 1]) + " = " + "{:<20}".format(y[special_i - 1])
            lines.append(s)
    else:
        lines.append("  none")
...
for label in labels:
    ...
    input_file  = f'test/{mode}/{label}_{mode}_solved.txt'
    ...
    for i in range(len(families)):
        ...
        for j in range(len(family_str)):
            if family_str[j][0] == "*": # each family contains exactly one special element
                family_to_special_x = family_str[j][1:]
                family_to_special.append(int(family_to_special_x))
                family_str[j] = family_to_special_x
        family = [int(s) for s in family_str]
        families[i] = family
    
    x_to_family = [None] * len(x) # x_to_family takes 0-indexing to 1-indexing
    for i in range(len(families)):
        for j in range(len(families[i])):
            x_to_family[families[i][j] - 1] = i + 1
    ...
    lines_top.append("chars where some sequence of nonzero coeffs is not unimodal in abs value:")
    formatter(i, nonunimodal_x, lines_top, x, y, x_to_family, family_to_special)
    lines_top.append("chars where some polynomial has both positive and negative coeffs:")
    formatter(i, mixed_x, lines_top, x, y, x_to_family, family_to_special)
    lines_top.append("chars where some polynomial has strictly negative coeffs:")
    formatter(i, negative_x, lines_top, x, y, x_to_family, family_to_special)
